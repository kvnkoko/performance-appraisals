...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'default', size = 'default', style, ...props }, ref) => {
    // Get the accent color for dynamic shadow
    const accentColor = typeof document !== 'undefined' 
      ? getComputedStyle(document.documentElement).getPropertyValue('--accent-color').trim() || '#3B82F6'
      : '#3B82F6';
    ...
    return (
      <button
        className={cn(
          ...
          {
            'gradient-primary text-white hover:scale-[1.02] active:scale-[0.98]': variant === 'default',
            ...
            'border border-border bg-transparent hover:bg-accent/10 hover:text-accent-foreground': variant === 'outline',
